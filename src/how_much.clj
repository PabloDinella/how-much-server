(ns how_much
  (:require [com.biffweb :as biff]
            [how_much.feat.app :as app]
            [how_much.feat.auth :as auth]
            [how_much.feat.home :as home]
            [how_much.schema :refer [malli-opts]]
...
(defn on-save [sys]
  ...
  (biff/eval-files! sys)
  (generate-assets! sys)
  (test/run-all-tests #"how_much.test.*"))
...
(def components
  [biff/use-config
   ...
   biff/use-chime
   (biff/use-when
    :how_much/enable-beholder
    biff/use-beholder)])

(defn start []
  (biff/start-system
   {:how_much/chat-clients (atom #{})
